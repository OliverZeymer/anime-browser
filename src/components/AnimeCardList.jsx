...
export default function AnimeCardList({ data, isRecommendations, isCharacter, className }) {
  return (
    <section className={!isCharacter && 'mt-6'}>
      <ul className={cn('sm:grid sm:grid-cols-auto-fit place-items-center place-content-center flex flex-col items-center sm:items-start gap-3 lg:gap-6 w-full h-fit', className)}>
        {data?.length === 0 || !data ? (
          <NoResultsFound />
        ) : (
          data?.map((anime) => (
